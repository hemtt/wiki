    ...
    I: Iterator<Item = char>,
{
    ...
    while let Some(&c) = chars.peek() {
        match c {
            ']' => {
                chars.next(); // Consume the ']'
                process_infinite_pattern(&mut array);
            ...
            _ => {
                if let Some(arg) = parse_arg(chars) {
                    array.push(arg);
                }
                if chars.peek() == Some(&',') {
                    chars.next(); // Consume the ','
...
fn process_infinite_pattern(array: &mut Vec<Arg>) {
        ...
        && last.trim() == "..."
    {
        ...
        let (pattern_items, count_to_remove) = extract_pattern(array);
...
fn extract_pattern(array: &[Arg]) -> (Vec<Arg>, usize) {
    ...
    if let Some((nested_pattern, count)) = extract_nested_array_pattern(array) {
